  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      if (boosts >= 30) maxbitrate = 384000;
        ...
        .addField(`<:arrow:832598861813776394> [${message.guild.roles.cache.size}] Roles: `, "> "+message.guild.roles.cache.size < 25 ? [...message.guild.roles.cache.values()].sort((a, b) => b.rawPosition - a.rawPosition).map(role => `<@&${role.id}>`).join(', ') : message.guild.roles.cache.size > 25 ? trimArray(message.guild.roles.cache) : 'None')
